(function() {
  ...
  const createWidget = () => {
    ...
    styles.textContent = `
      ...
      .accessibility-widget-panel::-webkit-scrollbar {
        width: 6px;
      }

      .accessibility-widget-panel::-webkit-scrollbar-track {
        background: #f1f5f9;
      }

      .accessibility-widget-panel::-webkit-scrollbar-thumb {
        background: #cbd5e1;
        border-radius: 3px;
      }

      .accessibility-widget-panel::-webkit-scrollbar-thumb:hover {
        background: #94a3b8;
      }
    `;
  ...
  const updateWidgetStyles = (settings) => {
    ...
    if (button && settings.buttonColor) {
      button.style.backgroundColor = settings.buttonColor;
      if (settings.buttonSize) {
        button.style.width = settings.buttonSize;
        button.style.height = settings.buttonSize;
      }
    }

    if (header && settings.headerColor) {
      header.style.backgroundColor = settings.headerColor;
      header.style.color = settings.headerTextColor || '#1e293b';
    }

    if (footer && settings.poweredByColor) {
      footer.style.color = settings.poweredByColor;
      footer.textContent = settings.poweredByText || 'Powered by Accessibility Widget';
    }

    if (container && settings.buttonPosition) {
      container.style.left = settings.buttonPosition.includes('left') ? '20px' : 'auto';
      container.style.right = settings.buttonPosition.includes('right') ? '20px' : 'auto';
      container.style.top = settings.buttonPosition.includes('top') ? '20px' : 'auto';
      container.style.bottom = settings.buttonPosition.includes('bottom') ? '20px' : 'auto';
    }
  };
